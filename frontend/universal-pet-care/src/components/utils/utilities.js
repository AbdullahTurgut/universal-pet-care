...
/* The purpose of this code is to generate a unique color based on the input string. The color is generated
using a hash function, which ensures that different input strings will result in different color
*/
...
export const generateColor = (str) => {
  ...
  It then loops through each character in the str parameter and performs  a hash 
  ...
  /* 3. Color Generation: 
  After the hash calculation, the function calculates the huge value by taking the hash value modula 360.
  This ensures that the hue value is within the range of 0 to 359 degrees, which represents the color wheel.
  The function then returns a color in the HSL (Hue, Saturation, Lightness) color format, using the calculated
  hue value and setting the saturation to 70% and the lightness to 50%.
   */
  const hue = hash % 360;
